...
namespace SonicRetro.SonLVL.API.SKC
{
	/// <summary>
	/// This format shouldn't be used outside of Sonic & Knuckles Collection.
	/// It shouldn't even be used in S&KC, but it is so we have to support it.
	/// </summary>
	public class Layout : LayoutFormatCombined
	{
			...
			for (int la = 0; la < Math.Max(fgh, bgh) * 4; la += 4)
			{
				...
				if (lfp != 0)
					for (int laf = 0; laf < fgw; laf++)
						layout.FGLayout[laf, la / 4] = rawdata[(lfp - StartAddress + laf) ^ 1];
				ushort lbp = ByteConverter.ToUInt16(rawdata, 8 + la + 2);
				if (lbp != 0)
					for (int lab = 0; lab < bgw; lab++)
						layout.BGLayout[lab, la / 4] = rawdata[(lbp - StartAddress + lab) ^ 1];
			}
		}
		...
		public override void WriteLayout(LayoutData layout, out byte[] rawdata)
		{
			...
			for (int y = 0; y < fgh; y++)
				for (int x = 0; x < fgw; x++)
					l.Add((byte)layout.FGLayout[x, y]);
			for (int y = 0; y < bgh; y++)
				for (int x = 0; x < bgw; x++)
					l.Add((byte)layout.BGLayout[x, y]);
